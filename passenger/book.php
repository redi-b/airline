...
uksort($seats_by_row, function ($a, $b) {
    return excel_row_index($a) - excel_row_index($b);
});
...
<section class="booking container">
    ...
    <div class="booking-grid">
        ...
        <form method="POST" action="" class="seat-selection-form">
            ...
            <div class="seat-map">
                <?php foreach ($seats_by_row as $row_label => $seats): ?>
                    <div class="seat-row">
                        <span class="row-number"><?php echo htmlspecialchars($row_label); ?></span>

                        <?php
                        $seat_index = 1;
                        foreach ($seat_layout as $group_index => $group_size):
                            ?>
                            <div
                                class="seat-group <?php echo $group_index === 0 ? 'left' : ($group_index === count($seat_layout) - 1 ? 'right' : 'middle'); ?>">
                                <?php for ($i = 0; $i < $group_size; $i++, $seat_index++): ?>
                                    <?php
                            ...
                            <?php if ($group_index < count($seat_layout) - 1): ?>
                                <div class="aisle"></div>
                            <?php endif; ?>
                        <?php endforeach; ?>
